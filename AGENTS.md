...
3. **Phase 3 EXE 模拟/实盘运行**：用于冻结候选后的 dry-run/demo/live runtime 打包、模拟下单、用户授权实盘、安全门、日志、订单意图和持仓恢复验证。打包必须符合 `MT5_RUNTIME_PACKAGING_STANDARD.md`，包括源码预检、直接 EXE 启动、外置配置、日志/对账、3000 根缓存、便携目录和 audit hook。REAL 账户不再被制度默认禁止；只要模拟/运行烟测通过，且用户明确授权 `live_trade` 模式、并满足与 demo 相同的技术门禁，即可进入受控实盘试运行。
...
3. **重启上下文。** 新版本实施应建立新的 Codex thread/对话或等价的上下文重启，并以 `NEW_VERSION_HANDOFF.md`、`PROJECT_STATE.md` 或版本目录 handoff 作为唯一上下文入口。旧版本长对话里的结论只能作为参考，不得作为当前版本事实来源。无法新建 thread 时，必须在输出中标记 `context_contamination_risk`。  
   **执行上下文清理。** 上下文重启前应创建本版本的 `CLEANUP_LOG.md` context section，清理并归档（移动到当前版本 `_trash_review/`）旧线程痕迹，包括：
   - 旧版本的 `PROJECT_STATE.md` 与 `NEW_VERSION_HANDOFF.md`（非当前版本）  
   - 旧版本 `CLEANUP_LOG.md` / `runtime_handoff.md` / `execution_audit.md` 的引用缓存  
   - 任何未标注版本的 loose note（`idea_note`、`temp_note`、`history_context`）  
   清理完成前不得进入正式实验输入；清理后可在 handoff 中写入 `context_reset_status: context_restarted_from_handoff` 或 `context_contamination_risk`，并引用归档位置。
...
### 12. 禁止跨版本读取回测数据、报告、缓存或记录（硬性阻断）

Phase 2+ 必须执行"一个版本，一个文件夹"。每个版本必须有独立 `versions/<version>/` 目录和 `version_manifest.yaml`；本版本的 backtest、report、audit、cache、log 只能写入并读取本版本目录。除带 hash 的只读原始市场数据快照外，不允许当前版本读取其他版本的 `backtests/`、`reports/`、`cache/`、`trades.csv`、`signals.csv` 或 loose `saved_runs`。  

版本对比只允许读取父版本的冻结摘要（如 `version_manifest.yaml`、`frozen_report`）作为参照文件，不得将历史版本的交易明细、缓存、信号或回测结果作为当前版本的计算输入。  

若发现以下任一情况，本轮正式研究结果直接标记 `version_isolation_unverified` 并 BLOCK：

- 扫描到 `versions/<other_version>/` 的跨版本路径引用（包括绝对路径和相对路径）；
- 发现 `latest`、`final`、`best`、`new`、`copy`、`副本`、`saved_runs` 等未绑定 version/hash 的输出命名；
- 代码、脚本或配置中出现读/写当前版本目录之外的 backtest/input/指标输入/运行产物；
- 新版本未在新子目录复制父版本 `.py` 并对该副本执行基线重审。

该条款违反直接阻断：不得声明为决策级或用于冻结/OOS/forward/live handoff。
...
11. **上下文是否已清理？** 对应新版本，必须确认旧版本上下文入口与旧版本 `PROJECT_STATE.md` 已归档到本版本 `_trash_review/<timestamp>/context/`，`CLEANUP_LOG.md` 中有 `context_cleanup` 区块记录。清理未完成则不得进行参数优化或冻结流程。
...
17. 开启新版本前，先完成上下文清理再继续逻辑开发：归档旧 `PROJECT_STATE.md`、旧 handoff 片段和非正式上下文笔记，确保当前版本的 `NEW_VERSION_HANDOFF.md` 与 `PROJECT_STATE.md`（如存在）只保留本版本最小必要条目。未清理上下文的版本不得进行手工或自动化对比、再建模、冻结或 runtime handoff。
