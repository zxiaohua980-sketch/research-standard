...
制度服务策略研究，不替代策略研究。Phase 1 允许轻门禁快速写代码、快测、盈亏比测试和归因；Phase 2 开始要求版本隔离、重新审计和必要记录；Phase 3 打包前最严格。

Phase 2+ 开启新版本时必须：

1. 在新的 `versions/<new_version>/` 子目录中复制父版本 active `.py`，形成新版本单独 `.py` 主文件；
2. 重新审计复制后的 baseline，再开始修改逻辑；
3. 使用新 thread/对话，或从 `NEW_VERSION_HANDOFF.md` 重启上下文并标记 `context_contamination_risk`；
4. 每轮结束清理临时、无效、半截输出，必要时放入 `_trash_review/` 并写 `CLEANUP_LOG.md`。

禁止删除原始数据、ledger、manifest、审计/归因/replay 报告、冻结/forward-live/runtime 证据、registry 或用户源文件，除非用户明确批准并有归档记录。明显临时文件、失败半截输出、未绑定 version/root/hash 的 loose `latest/final/copy/副本/saved_runs` 应及时清理。
...
任何对信号定义、入场规则、成交价约定、出场规则、止损/止盈/动态管理、头寸规模或成本模型的 Phase 2+ 改动都必须记录为新版本（v0.1 → v0.2）或新分支（main → experiment_branch）。新版本必须在新子目录复制父版本 active `.py` 为新版本单独 `.py`，重新审计，并使用新 thread/上下文 handoff。不允许在原版本上无声地改动。
...
10. **是否正在开启新版本？** 如果是 Phase 2+ 新版本，必须确认新子目录、复制后的 active `.py`、重新审计计划、新 thread/上下文 handoff 和 cleanup log。
...
5. 如果任务是 Phase 1 探索，优先写代码、快测和归因；缺完整 registry/ledger 时降级证据标签，不阻断研究。
6. 如果任务开启 Phase 2+ 新版本，先复制父版本 active `.py` 到新版本子目录、建立新 thread/handoff、重新审计 copied baseline。
7. 如果任务本身是审计、加固、修复未来函数、解释重放差异或批准候选，立即进入 `STRICT_AUDIT_ENFORCEMENT_STANDARD.md`：只做最小安全补丁，不做性能优化，不改变策略意图；任何修复后的指标必须重新生成并重新标注证据等级。
8. 如果任务涉及新增 filter，要求先提交 Trade Attribution 报告（Stage 5）。
9. 如果任务涉及 SL/TP、动态退出或头寸规模修改，先按 `EXIT_RISK_AND_LOGIC_REFINEMENT_STANDARD.md` 完成 exit/risk attribution、逻辑变更提案和重新审计。
10. 如果任务涉及参数修改，检查是否已完成 Stage 2、5、6，如果否，依次补齐。
...
13. 每轮结束清理临时/无效/半截输出；Phase 2+ 写 `CLEANUP_LOG.md` 或 cleanup section。
14. 如果任务涉及 forward-live 评估，检查是否已达 Gate（3个月+30笔或50笔）。
15. 任何规则修改都必须记录为新版本，更新 version.json、version_manifest.yaml；strategy_registry.yaml 只在里程碑变化时更新。
...
- 不允许用临时、半截、loose latest/final/copy/saved_runs 文件作为正式输入或证据。
