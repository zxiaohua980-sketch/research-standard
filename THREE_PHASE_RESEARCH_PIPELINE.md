...
新版本必须从父版本复制 active `.py` 主文件到新子目录，形成新版本独立代码入口。允许引用只读稳定库，但禁止修改父版本 active `.py` 或从父版本输出目录读取数据。新版本开发应使用新 Codex thread/对话；如果无法新建 thread，必须以 `NEW_VERSION_HANDOFF.md` 作为上下文重启点，并标记 `context_contamination_risk`。启动新版本前，需先将旧上下文文件（如 `PROJECT_STATE.md`、旧 handoff、旧 notes）归档到 `versions/<new_version>/_trash_review/context/` 并写入 `CLEANUP_LOG.md` 的 context section。
...
live_trade: 允许，前提是 dry-run/demo 或运行烟测通过，用户明确授权，并在 `live_trade` 模式下与 demo 相同技术门禁执行
```

### 运行安全底线

- REAL 账户不再默认拒绝；当用户明确授权并以 `live_trade` 运行、并且通过与 demo 相同的技术门禁时允许；
- `allow_live_trade`、`live_trade_ack` 保留为兼容字段，不再作为主门禁；
- demo/live 下单都必须显式授权；
- `kill_switch` 可阻断；
...
| REAL 下单 | 禁止 | 禁止 | 允许：用户明确授权 + live_trade 模式 + 与 demo 相同的运行安全检查通过 |
