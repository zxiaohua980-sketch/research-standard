...
8. 如果任务涉及正式新增 filter、SL/TP、动态退出或头寸规模修改，先按 Trade Attribution 和 `EXIT_RISK_AND_LOGIC_REFINEMENT_STANDARD.md` 完成归因、逻辑变更提案和重新审计。
...
11. Phase 2 冻结前必须执行逐根回测，并输出 signal/trade/equity diff；MTF 策略还必须输出 mtf_feature_diff。
12. 如果任务涉及 forward-live 评估，检查是否已达 Gate（3个月+30笔或50笔）。
13. 任何 Phase 2+ 规则修改都必须记录为新版本，更新 version.json、version_manifest.yaml 和 strategy_registry.yaml。
