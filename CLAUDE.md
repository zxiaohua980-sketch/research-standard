...
本文档是 Claude Code、Codex 以及所有跨目录研究流程的最高优先级约束。任何策略任务必须遵循以下十大禁区，违反任何一条意味着该研究结果无效。涉及数据切分与 OOS 声明时必须遵循 `DATA_SPLIT_AND_OOS_POLICY.md`；涉及成交、止损、止盈、移动止损、头寸规模或实盘保护时，还必须遵循 `EXIT_RISK_AND_LOGIC_REFINEMENT_STANDARD.md`。
...
8. 如果任务涉及 forward-live 评估，检查是否已达 Gate（3个月+30笔或50笔）。
9. 任何规则修改都必须记录为新版本，更新 version.json 和 strategy_registry.yaml。
