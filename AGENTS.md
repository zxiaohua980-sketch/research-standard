...
本文档是 Codex、Claude Code 以及所有跨目录研究流程的最高优先级约束。任何策略任务必须遵循以下十二大禁区，违反任何一条意味着该研究结果无效。涉及数据切分与 OOS 声明时必须遵循 `DATA_SPLIT_AND_OOS_POLICY.md`；涉及成交、止损、止盈、移动止损、头寸规模或实盘保护时，还必须遵循 `EXIT_RISK_AND_LOGIC_REFINEMENT_STANDARD.md`；涉及多周期特征、逐根回测、版本目录、输出文件或跨版本对比时，还必须遵循 `MTF_LOOKAHEAD_AND_VERSION_ISOLATION_STANDARD.md`。
...
3. **Phase 3 EXE 模拟下单**：用于冻结候选后的 dry-run/demo runtime 打包、模拟下单、安全门、日志、订单意图和持仓恢复验证。REAL 账户下单不属于本制度允许范围。
...
任何新策略项目进入 Phase 2 正式候选迭代前，必须在 D:\MT5\research_registry\strategy_registry.yaml（或 fallback registry）中登记，包括 strategy_id、root_path、current_stage、current_status。Phase 1 探索可以先使用 `idea_card.md`，但不得声称正式研究、OOS 或 forward-live。registry 是跨目录协调的唯一权威记录。
...
在执行任何策略相关代码修改、参数调整、数据分析时，必须先回答以下九个问题：
...
6. 如果任务涉及新的数据或正式逻辑，要求 Execution Audit 报告（Stage 2）；Phase 1 只允许致命审计快测。
7. 如果任务涉及正式新增 filter、SL/TP、动态退出或头寸规模修改，先按 Trade Attribution 和 `EXIT_RISK_AND_LOGIC_REFINEMENT_STANDARD.md` 完成归因、逻辑变更提案和重新审计。
8. 如果任务涉及多周期、重采样或高周期特征，Phase 2+ 必须输出 MTF 时间可用性审计；没有通过时不得进入正式回测结论。
9. Phase 2+ 的每个候选必须有独立 version_root 和 `version_manifest.yaml`，并通过版本隔离检查；不得跨版本读取回测输出。
10. Phase 2 冻结前必须执行逐根回测，并输出 signal/trade/equity diff；MTF 策略还必须输出 mtf_feature_diff。
11. 如果任务涉及 forward-live 评估，检查是否已达 Gate（3个月+30笔或50笔）。
12. 任何 Phase 2+ 规则修改都必须记录为新版本，更新 version.json、version_manifest.yaml 和 strategy_registry.yaml。
